...
public class DictionaryServiceImpl extends BaseServiceImpl<Dictionary, Integer> implements DictionaryService{
	...
	public List<Map<String, Object>> findByTypeId(Integer typeId, Integer parentId) {
		...
		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(Dictionary.class).createAlias("dictionaryType", "dictionaryTypeAlias", JoinType.LEFT_OUTER_JOIN);
	...
	public List<Map<String, Object>> findSubSet(Integer parentId, Integer gpId) {
		...
		for (Dictionary dictionary : dictionarieList) {
			...
			map.put("dataItemId", dictionary.getDataItemId());
			map.put("gpId", gpId);
			modifyDate = dictionary.getModifyDate();
			if (modifyDate != null) {
				map.put("modifyDate", new SimpleDateFormat("yyyy-MM-dd").format(modifyDate));
			} else {
				map.put("modifyDate", "");
			}
			list.add(map);
	...
	public int deleteByIds(Integer[] ids) {
		...
		String hql="SELECT PRIKEY FROM T_SYS_DICTIONARY WHERE PRIKEY in (:ids)  " +
		" union all " +
		" select PRIKEY from T_SYS_DICTIONARY where PARENTID in (:ids) " +
		" union all " +
	...
	public Integer[] findSonId(Integer id) {
		...
		String hql="SELECT PRIKEY FROM T_SYS_DICTIONARY WHERE PRIKEY = :id  " +
		" union all " +
		" select PRIKEY from T_SYS_DICTIONARY where PARENTID = :id " +
		" union all " +
		" select PRIKEY from T_SYS_DICTIONARY where PARENTID in (select PRIKEY from T_SYS_DICTIONARY where PARENTID = :id)";
		Query query = getSession().createSQLQuery(hql);
